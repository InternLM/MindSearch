...
from msdl.config import (BACKEND_DOCKERFILE_DIR, CLOUD_LLM_DOCKERFILE,
                         FRONTEND_DOCKERFILE_DIR, LOCAL_LLM_DOCKERFILE,
                         PACKAGE_DIR, PROJECT_ROOT, REACT_DOCKERFILE, TEMP_DIR,
                         TEMPLATE_FILES)
from msdl.docker_manager import (check_docker_install, run_docker_compose,
                                 stop_and_remove_containers,
                                 update_docker_compose_paths)
from msdl.i18n import setup_i18n, t
from msdl.utils import (clean_api_key, copy_templates_to_temp,
                        get_existing_api_key, get_model_formats,
                        modify_docker_compose, save_api_key_to_env,
                        validate_api_key)
    ...
    if model == CLOUD_LLM_DOCKERFILE:
        ...
        if existing_api_key:
            use_existing = inquirer.confirm(
                message=t('CONFIRM_USE_EXISTING_API_KEY',
        ...
        while True:
            api_key = inquirer.secret(
                message=t('PLEASE_INPUT_NEW_API_KEY_FROM_ZERO',
            ...
            if validate_api_key(cleaned_api_key, env_var_name, t):
                ...
                retry = inquirer.confirm(message=t('RETRY_API_KEY_INPUT'),
