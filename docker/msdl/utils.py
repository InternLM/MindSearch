...
import yaml
from msdl.config import (CLOUD_LLM_DOCKERFILE, ENV_FILE_PATH,
                         LOCAL_LLM_DOCKERFILE, PACKAGE_DIR, TEMP_DIR)
...
def modify_docker_compose(selected_dockerfile, backend_language, model_format):
    ...
    if selected_dockerfile == CLOUD_LLM_DOCKERFILE:
        ...
        raise ValueError(
            t('UNKNOWN_DOCKERFILE', dockerfile=selected_dockerfile))
    ...
    print(
        t(
            ...
            mode=(t('CLOUD') if selected_dockerfile == CLOUD_LLM_DOCKERFILE
                  else t('LOCAL')),
            format=model_format,
        ))
