...
def check_docker_install():
    try:
        ...
        subprocess.run(docker_compose_cmd + ['version'],
                       check=True,
                       capture_output=True)
...
def stop_and_remove_containers():
    ...
    try:
        ...
        project_containers = [
            c for c in all_containers if c.startswith(container_prefix) or any(
                c == f"{project_name}-{service}" for service in service_names)
        ]
            ...
            for container in project_containers:
                try:
                    ...
                    subprocess.run(['docker', 'rm', '-f', container],
                                   check=True)
                    print(
                        t('CONTAINER_STOPPED_AND_REMOVED',
                          container=container))
                except subprocess.CalledProcessError as e:
                    print(
                        t('CONTAINER_STOP_ERROR',
                          container=container,
                          error=str(e)))
    ...
    for service in compose_data['services'].values():
        if 'build' in service:
            ...
            if 'dockerfile' in service['build']:
                dockerfile_name = os.path.basename(
                    service['build']['dockerfile'])
                service['build']['dockerfile'] = os.path.join(
                    TEMP_DIR, dockerfile_name)
